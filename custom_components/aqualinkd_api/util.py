...
def _is_ignored_name(name_or_id: str) -> bool:
    if not name_or_id:
        return False
...
def _extract_containers_from(d: dict[str, Any], devices: dict[str, dict[str, Any]]) -> None:
    for container in ("leds", "timers", "sensors"):
        if container in d and isinstance(d[container], dict):
            ...
            for k, v in d[container].items():
                if _is_ignored_name(k):
                    continue
                devices[str(k)] = {"id": k, "name": k, "state": v}


def _process_list_source(src: list[Any], devices: dict[str, dict[str, Any]]) -> None:
    ...
    for item in src:
        if isinstance(item, dict):
            ...
            if name and not _is_ignored_name(name) and not _is_ignored_name(item.get("id")):
                devices[str(name)] = dict(item)
        elif item is not None and not _is_ignored_name(str(item)):
            devices[str(item)] = {"name": str(item), "state": item}


def _process_dict_source(src: dict[str, Any], devices: dict[str, dict[str, Any]]) -> None:
    _LOGGER.debug("Processing dict source with %d keys", len(src))
    for key, value in src.items():
        if _is_ignored_name(key):
            continue
        if isinstance(value, dict):
            if _is_ignored_name(value.get("id")) or _is_ignored_name(value.get("name")):
                continue
            device = dict(value)
