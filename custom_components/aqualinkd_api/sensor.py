...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry, async_add_entities: AddEntitiesCallback) -> None:
    ...
    for pump_name in coordinator.data.pump_filtered:
        ...
        entities.extend([
            ...
            PumpFilteredSensor(coordinator, pump_name, "filter_state", "Telemetry Filter State"),
        ])
        # Mark these as handled so the generic loop doesn't create duplicates
        handled_attrs.add((pump_name, "rpm"))
        handled_attrs.add((pump_name, "watts"))
        handled_attrs.add((pump_name, "filter_state"))
        
        # ALSO mark the raw API keys as handled to prevent generic sensors
        handled_attrs.add((pump_name, "Pump_RPM"))
        handled_attrs.add((pump_name, "Pump_Watts"))
        handled_attrs.add((pump_name, "Pump_GPM"))
        handled_attrs.add((pump_name, "raw_rpm"))
        handled_attrs.add((pump_name, "raw_watts"))
        handled_attrs.add((pump_name, "raw_gpm"))
        
        # We no longer create raw pump sensors here as they clutter the UI.
        # Use the standard filtered sensors instead.
    ...
    for name, dev in coordinator.data.devices.items():
        ...
        for key, val in dev.items():
            ...
            if has_real_value and key in {"state", "status", "int_status"}:
                _LOGGER.debug("Skipping generic state key %s for %s as it has a real value attribute", key, name)
                continue
